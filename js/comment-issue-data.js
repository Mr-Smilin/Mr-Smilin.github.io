	...
	if (COMMENT_ARR.length > 0) {
		...
		for (var i = 0; i < COMMENT_ARR.length; i++) {
			...
			htmlContentWidget +=
				...
				"<img class='ava' src='" +
				item.userAvatar +
				"'  onerror='this.onerror = null;this.src=\"https://cdn.jsdelivr.net/npm/gitalk@1/src/assets/icon/github.svg\";' /></a>" +
...
function loadIndexHotData(authorizationToken) {
	...
	if ($("#index_hot_div").length > 0) {
		...
		ajaxReqForGitHub(
			repoIssuesUrl + "?per_page=10&sort=comments",
			authorizationToken,
			function (result) {
				$.each(result, function (i, item) {
					if (item.body != null) {
		...
		valine
			...
			.then(function (comments) {
				...
				for (var i = 0; i < comments.length; i++) {
					...
					var avatar =
						"https://gravatar.loli.net/avatar/" + md5(mail) + "?d=mp";
					...
					resultArr.push({
						content: content,
						date: new Date(createDate.getTime() - 8 * 1000 * 60 * 60).Format(
...
function loadIssueData(appId, appKey, userName, userRepo, isValine) {
	setTimeout(function () {
		...
		if (isValine) {
			...
			repoIssuesUrl = repoIssuesUrl
				.replace("userName", userName)
				.replace("userRepo", userRepo);
			ADMIN_NAME = userName;
			clientId = appId;
			clientSecret = appKey;
